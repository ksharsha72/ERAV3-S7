import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim


class Model(nn.Module):
    def __init__(self):
        super(Model, self).__init__()

        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 8, 3, padding=1), nn.ReLU(), nn.Dropout(0.1), nn.BatchNorm2d(8)
        )  # 28, 3
        self.conv2 = nn.Sequential(
            nn.Conv2d(8, 10, 3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(10),
        )  # 28, 5
        self.pool1 = nn.MaxPool2d(2, 2)  # 14, 6
        self.ant1 = nn.Sequential(
            nn.Conv2d(10, 8, 1), nn.ReLU(), nn.Dropout(0.1), nn.BatchNorm2d(8)
        )  # 14, 6
        self.conv3 = nn.Sequential(
            nn.Conv2d(8, 12, 3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(12),
        )  # 14, 10
        self.conv4 = nn.Sequential(
            nn.Conv2d(12, 16, 3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(16),
        )  # 14 , 14
        self.pool2 = nn.MaxPool2d(2, 2)  # 7, 16
        self.ant2 = nn.Sequential(
            nn.Conv2d(16, 8, 1), nn.ReLU(), nn.Dropout(0.1), nn.BatchNorm2d(8)
        )  # 7, 16
        self.conv5 = nn.Sequential(
            nn.Conv2d(8, 10, 3, padding=1),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.BatchNorm2d(10),
        )  # 7, 24
        self.conv6 = nn.Sequential(
            nn.Conv2d(10, 12, 3), nn.ReLU(), nn.Dropout(0.1), nn.BatchNorm2d(12)
        )  # 5 , 32
        self.avgpool = nn.AvgPool2d(5)
        self.conv7 = nn.Conv2d(12, 10, 1)

    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.pool1(x)
        x = self.ant1(x)
        x = self.conv3(x)
        x = self.conv4(x)
        x = self.pool2(x)
        x = self.ant2(x)
        x = self.conv5(x)
        x = self.conv6(x)
        x = self.avgpool(x)
        x = self.conv7(x)
        x = x.view(-1, 10)
        return F.log_softmax(x, dim=-1)
